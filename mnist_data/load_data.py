...
train_output_dir = 'saved_images/train'
test_output_dir = 'saved_images/test'
...
for idx, (img, label) in enumerate(zip(x_train, y_train)):
    filename = f"{train_output_dir}/train_{idx:05d}_label_{label}.png"
    image_to_save = Image.fromarray(np.uint8(img))
    image_to_save.save(filename)
print("Imágenes de entrenamiento guardadas.")

# Guardar imágenes de prueba
print("Guardando imágenes de prueba...")
for idx, (img, label) in enumerate(zip(x_test, y_test)):
    filename = f"{test_output_dir}/test_{idx:05d}_label_{label}.png"
    image_to_save = Image.fromarray(np.uint8(img))
    image_to_save.save(filename)
