...
#pragma omp parallel for collapse(3) schedule(static)
  ...
  // Convierte índice plano -> índices múltiples (para N-dims)
  std::vector<int> unravel_index(int idx, const std::vector<int> &shape) const
  {
  ...
  // Convierte índices múltiples -> índice plano
...
#pragma omp simd reduction(+ : sum)
  ...
  Tensor sum(const std::vector<int> &dims) const
  {
    ...
    Tensor result(new_shape);
    const int result_size = result.data.size();
...
#pragma omp parallel
    {
      // Cada hilo tiene su propio buffer temporal
      std::vector<float> local_result(result_size, 0.0f);

#pragma omp for nowait
      for (int idx = 0; idx < total; ++idx)
      {
        std::vector<int> idx_multi = unravel_index(idx, shape);
        std::vector<int> reduced_idx;

        for (size_t i = 0; i < idx_multi.size(); ++i)
        {
          if (!reduce_axes[i])
            reduced_idx.push_back(idx_multi[i]);
        }

        int flat = ravel_index(reduced_idx, result.shape);
        local_result[flat] += data[idx];
      }
...
#pragma omp critical
  ...
  void reshape_inplace(const std::vector<int> &new_shape)
  {
    int total = 1;
    for (int d : new_shape)
      total *= d;
    assert(total == data.size()); // verificar compatibilidad
    shape = new_shape;
  }
...
#pragma omp parallel for collapse(3) schedule(static)
  ...
  Tensor operator-(const Tensor &other) const
  {
    ...
    {
      std::vector<int> idx = unravel_index(i, shape); // tu función para convertir posición plana a índices

      int flat_index_self = i;
      int flat_index_other = 0;
      std::vector<int> idx_other(other.shape.size());

      for (size_t d = 0; d < shape.size(); ++d)
      {
        idx_other[d] = (other.shape[d] == 1) ? 0 : idx[d];
      }

      flat_index_other = ravel_index(idx_other, other.shape); // tu función inversa
      result.data[i] = data[i] - other.data[flat_index_other];
    }
...
#pragma omp parallel for
    ...
    {
      std::vector<int> idx = unravel_index(i, shape); // [d0, d1, ..., dn]

      std::vector<int> idx_other(other.shape.size());
      for (size_t d = 0; d < shape.size(); ++d)
      {
        idx_other[d] = (other.shape[d] == 1) ? 0 : idx[d]; // broadcasting
      }

      int flat_other = ravel_index(idx_other, other.shape);
